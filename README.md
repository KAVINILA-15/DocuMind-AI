...
├── generate\_ipynb.py            # Generates/export notebook

├── loader.py                    # PDF loading functions

├── rag\_pipeline.py              # Complete RAG workflow pipeline
...
├── faiss\_index/                 # Stored vector database
...
|What are the four types of reflection tokens? |Correct|Matches document content|
...
|What does BERT stand for?|Not answered|Not present in PDF|
...
### `404 NOT\\\\\\\_FOUND` for Gemini embedding model
...
### `RESOURCE\\\\\\\_EXHAUSTED` / 429 Rate Limit Error
...
* The **FAISS index** is saved locally in the `faiss\\\\\\\_index/` folder
