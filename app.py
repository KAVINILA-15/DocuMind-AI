...
from rag_pipeline import generate_answer, fix_spelling
...
# ─── Sidebar ──────────────────────────────────────────────────────────────────
# Hardcoded generation model — no sidebar selector needed
...
with st.sidebar:
    ...
    chunk_strategy = st.radio(
        "Select:",
        options=["A", "B"],
        format_func=lambda x: "A — size 700, overlap 80" if x == "A" else "B — size 1000, overlap 100",
    )
    ...
    with st.chat_message(message["role"]):
        ...
        if message.get("citations"):
            with st.expander("📚 Show Top 3 Sources"):
                for i, cit in enumerate(message["citations"][:3], 1):
                    st.markdown(
    ...
    if not os.getenv("GOOGLE_API_KEY"):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking…"):
                try:
                    if prompt in st.session_state.answer_cache:
                        cached_result = st.session_state.answer_cache[prompt]
                        ...
                        retriever = get_retriever(strategy=retrieval_strategy, k=5)
                        docs = retriever.invoke(corrected_prompt)

                        # Format context + citations
                        formatted_context, citations = format_docs_for_citation(docs)

                        # Generate answer using hardcoded model
                    ...
                    if "429" in err or "RESOURCE_EXHAUSTED" in err:
                        st.error(
                            f"⚠️ **Gemini Rate Limit Reached (429)**\n\n"
                            f"Your free-tier quota for **`{GEMINI_MODEL}`** is exhausted.\n\n"
                            f"**Fix:** Wait a moment and try again, or upgrade your Google AI plan."
                        )
                    else:
                        st.error(f"Error: {e}")
