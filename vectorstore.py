...
def create_vector_store(
    ...
    try:
        ...
        if embedding_method == "gemini":
            ...
            st.error(
                f"⚠️ **Gemini embedding failed** — falling back to HuggingFace.\n\n"
                f"**Reason:** `{type(e).__name__}: {e}`\n\n"
                f"**Fix:** Ensure your `GOOGLE_API_KEY` is set in the `.env` file or the sidebar."
            )
            embedding_method = "huggingface"
            embeddings = get_embedding_model(method="huggingface")

            # Dimensions differ between Gemini and HuggingFace, so always rebuild on fallback
            if os.path.exists(persist_directory):
                shutil.rmtree(persist_directory)
            vector_store = FAISS.from_documents(chunks, embeddings)
        else:
